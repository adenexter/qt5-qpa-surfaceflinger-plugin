...
**
** Based on eglfshwcintegration.h
**
...
#define SURFACEFLINGER_PLUGIN_EXPECT_ZERO(x) \
    { int res; if ((res = (x)) != 0) \
        qCWarning(QPA_LOG_SURFACEFLINGER, "%s in %s returned %i", (#x), __func__, res); }

// Evaluate "x", if it isn't NULL, print a warning
#define SURFACEFLINGER_PLUGIN_EXPECT_NULL(x) \
    { void *res; if ((res = (x)) != NULL) \
        qCWarning(QPA_LOG_SURFACEFLINGER, "%s in %s returned %x", (#x), __func__, (unsigned int)res); }

// Evaluate "x", if it is NULL, exit with a fatal error
#define SURFACEFLINGER_PLUGIN_FATAL(x) \
    qCFatal(QPA_LOG_SURFACEFLINGER, "%s in %s", x, __func__)

// Evaluate "x", if it is NULL, exit with a fatal error
#define SURFACEFLINGER_PLUGIN_ASSERT_NOT_NULL(x) \
    { void *res; if ((res = (x)) == NULL) \
        qCFatal(QPA_LOG_SURFACEFLINGER, "%s in %s returned %x", (#x), __func__, (unsigned int)res); }

// Evaluate "x", if it doesn't return zero, exit with a fatal error
#define SURFACEFLINGER_PLUGIN_ASSERT_ZERO(x) \
    { int res; if ((res = (x)) != 0) \
        qCFatal(QPA_LOG_SURFACEFLINGER, "%s in %s returned %i", (#x), __func__, res); }
        ...
        QPlatformWindow *platformWindow, const QSize &size, const QSurfaceFormat &format)
{
    ...
    if (sf_surface) {
        SURFACEFLINGER_PLUGIN_FATAL("There can only be one window, someone tried to create more.");
    }
...
{
    if (display_off) {
        qCWarning(QPA_LOG_SURFACEFLINGER, "Swap requested while display is off");
        return true;
    }
...
void *EglFsSurfaceFlingerIntegration::nativeResourceForIntegration(const QByteArray &resource)
{
    ...
    if (lowerCaseResource == "displayoff") {
        ...
        qCDebug(QPA_LOG_SURFACEFLINGER, "sleepDisplay");
        display_off = true;
        ...
        qCDebug(QPA_LOG_SURFACEFLINGER, "unsleepDisplay");
        display_off = false;
